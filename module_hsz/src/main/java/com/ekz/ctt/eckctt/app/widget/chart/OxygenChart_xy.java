...
public class OxygenChart_xy {
	private static OxygenChart_xy graph;
	private static List<HealthBean> oxygenList = new ArrayList<HealthBean>();
	public static OxygenChart_xy getInstance() {
		if (graph == null) {
			graph = new OxygenChart_xy();
		}
		return graph;
	}
	...
	public static View getWeekChartView(Context context,
                                        List<HealthBean> temptureGradeList,
                                        String tag) {
		
		oxygenList = temptureGradeList;
		...
		for (int i = 0; i < oxygenList.size();i++) {
			renderer.addXTextLabel(i+1,oxygenList.get(i).getCreateTime2());
		}
		...
		xyRenderer.setColor(Color.parseColor("#22ad38"));//22ad38
		...
		xyRenderer1.setChartValuesTextSize(18f);
	...
	private static List<Float> getOxygenList(List<HealthBean> valuse)
	{
	...
	private static List<Float> getPluserateList(List<HealthBean> valuse)
	{
		List<Float> temp = new ArrayList<Float>();
		for(int i=0;i<valuse.size();i++)
		{
	...
	private static double getMaxValue(List<HealthBean> valuse){
		double max = 0.0;
		for(int i=0;i<valuse.size();i++){
			HealthBean oxygen = valuse.get(i);
	...
	private static double getMinValue(List<HealthBean> valuse){
		double min = 0.0;
		for(int i=0;i<valuse.size();i++){
			HealthBean oxygen = valuse.get(i);
			double oxy = oxygen.oxy;
