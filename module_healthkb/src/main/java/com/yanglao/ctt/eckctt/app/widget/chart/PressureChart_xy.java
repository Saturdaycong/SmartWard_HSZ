...
public class PressureChart_xy {
	...
	private static List<HealthServerBean> sHealthServerBeanList;
	...
	public static View getWeekChartView(Context context,
										List<HealthServerBean> healthServerBeanList,
										String tag) {
		sHealthServerBeanList = healthServerBeanList;
		if (sHealthServerBeanList==null) return null;
		...
		if(healthServerBeanList.size()>0) {
			renderer.setYAxisMin(getMinValue(healthServerBeanList));    //设置y轴最小值是35
			renderer.setYAxisMax(getMaxValue(healthServerBeanList) * 1.2);    //设置y轴最大值是260
		}else{
		...
		for (int i = 0; i < healthServerBeanList.size();i++) {
			renderer.addXTextLabel(i+1,healthServerBeanList.get(i).getCreateTime() );
		}
		
		renderer.setPanLimits(new double[]{0, healthServerBeanList.size()+0.5, 0, 0});//设置拖动的最小值最大值
		if(healthServerBeanList.size() > 7){
			renderer.setRange(new double[]{healthServerBeanList.size()-7.5, healthServerBeanList.size()+0.5, -10d, 240d}); //设置chart的视图范围
		}
...
//		xyRenderer.setNormalColor("#8ec21f");
		xyRenderer.setNormalColor("#008aff");
		xyRenderer.setyListAxisValuses(getYMaxValuse(healthServerBeanList));
		...
		xyRenderer1.setyListAxisValuses(getYMinValuse(healthServerBeanList));
	...
	public static XYMultipleSeriesDataset getXYMultipleSeriesDataset(String tag) {
		...
		for (int i = 1; i < sHealthServerBeanList.size() + 1; i++) {
			String tempobj =	sHealthServerBeanList.get(i - 1).heightPressure+"";
			Double temp    = Double.parseDouble(tempobj);
			series.add(i,temp);
			//////////////////////////////////////////////////////////
			String tempobj2 =sHealthServerBeanList.get(i - 1).lowPressure+"";
	...
	private static List<Float> getYMaxValuse(List<HealthServerBean> valuse)
	{
		List<Float> temp = new ArrayList<Float>();
		for(int i=0;i<valuse.size();i++)
		{
			temp.add(Float.valueOf(valuse.get(i).heightPressure));
		}
		return temp;
	}
	
	private static List<Float> getYMinValuse(List<HealthServerBean> valuse)
	{
		List<Float> temp = new ArrayList<Float>();
		for(int i=0;i<valuse.size();i++)
		{
			temp.add(Float.valueOf(valuse.get(i).lowPressure));
		}
		return temp;
	}
	...
	private static double getMaxValue(List<HealthServerBean> valuse){
		double max = 0.0;
		for(int i=0;i<valuse.size();i++){
			double ssy = Double.parseDouble(valuse.get(i).heightPressure);
			double szy = Double.parseDouble(valuse.get(i).lowPressure);
			if(ssy > max){
				max = ssy;
			}
			if(szy > max){
